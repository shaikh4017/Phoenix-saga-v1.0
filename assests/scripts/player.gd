...
# --- Camera ---
@onready var camera: Camera2D = $Camera2D  # child of player
...
@export var scene_change_x: float = 1400  # world X-coordinate where scene should change
...
func _physics_process(delta: float) -> void:
	...
	# --- Scene change if player passes trigger X-coordinate ---
	if global_position.x > scene_change_x:
		print("Player passed trigger X, switching to option.tscn")
